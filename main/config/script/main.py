...
def archiveIndex(index):
    ...
    dump_map = binary_path + " --fsCompress --limit=10000 --delete --input=http://" + host + ":" + str(port) + "/" + index + " --output=" + \
               archive_log_path + index + "-map.gz --type=mapping"
    dump_data = binary_path + " --fsCompress --delete --limit=10000 --input=http://" + host + ":" + str(port) + "/" + index + " --output=" + \
